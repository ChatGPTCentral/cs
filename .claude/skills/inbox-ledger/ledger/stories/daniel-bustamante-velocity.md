# Daniel Bustamante (Velocity)

- query: none - no Gmail label. Found by Notion call sweep, 2026-08-26
- label-id: none
- kind: project (outbound growth pitch, one contact)
- people: Daniel Bustamante, founder of Velocity (email growth/ghostwriting
  agency)
- status: open - agreed to a one-off audit
- start: 2026-04-29
- end: (open)
- next-action: none currently tracked
- commitments: an audit was agreed, scope/date not confirmed in the
  transcript
- threads: none - found via Notion call transcript, not Gmail
- notes: Alex described AI Central/Essential Media's revenue as
  $10-20K/month, mostly ads (~half programmatic beehiiv, half direct brand
  deals), a $4-trial-then-$40/year digital product, affiliates,
  co-registrations and consulting - targeting $50-80K/month by year end
  and a SaaS platform launch mid-June. Called his welcome sequence
  "completely fucked up" and mentioned having talked to Manny Reyes (see
  manny-reyes-boletin) the prior week. Daniel's pricing starts around $5K,
  up to $10-15K depending on scope; they agreed to a one-off audit rather
  than full beehiiv account access.

  **Correction, 2026-08-26.** This note previously said Alex had just
  returned from paternity leave for a first child named Luca. Alex
  confirmed directly that Luca is Daniel Bustamante's newborn, not his -
  the call opens with Daniel apologizing for running late because he was
  putting the baby to sleep, and Alex asking about the name. Notion's own
  AI-generated call summary attributed Daniel's news to Alex; a research
  agent repeated that misattribution without checking speaker identity
  against the raw transcript, and it went into this file unverified.
  Real lesson: a synthesized call summary can swap who-said-what between
  participants, especially for a personal aside early in a call - verify
  the speaker, not just the fact, before writing anything personal about
  Alex into this ledger.
