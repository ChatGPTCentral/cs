# Pillsbury Law

**confirmed, 2026-08-26, per Alex.** Not a standalone story - Pillsbury Law
hosted an event during London Tech Week 2026 that Elizabeth and Alex
attended in person, collecting contacts there. The 4 June correspondence
below falls inside the LTW window (3-11 June), which lines up. On
`/genesis` this is now a sub-event of `london-tech-week-2026`, not its own
top-level story.

- query: none - no label. Found in the January-to-August widening pass
- label-id: none
- kind: org
- people: ed.cavazos@pillsburylaw.com, steven.james@pillsburylaw.com,
  hattie.winfield@pillsburylaw.com - all @pillsburylaw.com
- status: **theirs**
- last-inbound: 2026-06-04 (Steven.James@pillsburylaw.com, cc Hattie Winfield)
- last-outbound: 2026-06-04 (alex@thecentral.ai)
- idle: 76 days
- next-action: none unless reviving
- commitments: none recorded
- threads: `19e940512c71cb60`, `19e9400f3d468989`, plus `19f27c84658a8347`
  (Ed Cavazos, part of the `interviews` outreach batch)
- notes: three people at one law firm, which usually means either legal
  counsel or a partnership/deal in motion. Ed Cavazos also appears in the
  `interviews` guest-outreach batch, so this may be two distinct relationships
  (an interview lead, and a separate legal/partnership contact) that happen to
  share a domain - not merged, flagged for a body read before assuming either way
