...
export default async function NetworkPage() {
  const [people, stories, companies] = await Promise.all([
    supabaseSelect("ledger_people", "?archived=eq.false&select=id,name,org,stories,starred,photo_url,company_id&order=name.asc"),
  ...
  const { nodes, links, orphanCount } = buildKnowledgeGraph(stories, people, companies);
  ...
  return (
    <div className="content wide-content">
      ...
      <p style={{ fontSize: 13, color: "var(--ink-faint)", margin: "0 0 14px" }}>
        Tutta la genesi come grafo vivo: storie e aziende sono i nodi grandi,
        le persone i puntini collegati a dove compaiono. Passa sopra un nodo
        per illuminare il suo vicinato, cerca un nome per trovarlo, clicca
